...
#if defined(NDEBUG)
# error "Hellascoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfda4fa9557e4b02822a6099858cdbfee4f49736e8a8395d1d87beba4d1285b28");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hellascoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hellascoin Signed Message:\n";
...
{
    // Hellascoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Hellascoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 28800; // Hellascoin: Every 3.5 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hellascoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hellascoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Hellascoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfda4fa9557e4b02822a6099858cdbfee4f49736e8a8395d1d87beba4d1285b28");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Gikas Hardouvelis Minister of Finances";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb91581556da2ab679f303f63e63a322eab2429ba88e8197b7d6419d79149a461"));
        //// start
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Hellascoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HellascoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HellascoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HellascoinMiner(CWallet *pwallet)
{
    printf("HellascoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hellascoin-miner");
    ...
    try { loop {
        ...
        printf("Running HellascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HellascoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HellascoinMiner, pwallet));
}
